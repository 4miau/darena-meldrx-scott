...
using MeldRx.Account.Helpers.Localization;
using MeldRx.Account.ViewModels.Manage;
...
namespace MeldRx.Account.Controllers
{
    ...
    public class ManageController: Controller
    {
        ...
        public async Task<IActionResult> Index(IndexViewModel model)
        {
            ...
            if (model.Email != email && string.IsNullOrEmpty(email))
            {
            ...
            var modifyUserDto = new ModifyApplicationUserDto
            {
                FirstName = model.FirstName,
                LastName = model.LastName
            };

            var (result, modifiedUser) = await _accountManager.ModifyUserPropertiesAsync(user, modifyUserDto);
            if (result.Succeeded)
            {
                var auth = await HttpContext.AuthenticateAsync((User.Identity as ClaimsIdentity)!.AuthenticationType);
                var claims = new List<Claim>();
                var clientIdClaim = User.FindFirst(JwtClaimTypes.ClientId);
                if (clientIdClaim != null)
                {
                    claims.Add(clientIdClaim);
                }
                var scopeClaims = User.FindAll(JwtClaimTypes.Scope).ToList();
                if (scopeClaims.Count > 0)
                {
                    claims.AddRange(scopeClaims);
                }
                await _signInManager.SignInWithClaimsAsync(modifiedUser, auth.Properties, claims);
            }
        ...
        private string GenerateQrCodeUri(string email, string unformattedKey)
        {
            return string.Format(
                ...
                _urlEncoder.Encode("MeldRx.Account"),
