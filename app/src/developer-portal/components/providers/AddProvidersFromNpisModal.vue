...
const emit = defineEmits<{
  'close': [];
  'create': [CreateWorkspaceProvidersFromNpisCommand];
}>();
...
function onAddProviderClick() {
    ...
    emit('create', {
        npis: state.form.npis.match(/\d{10}/g) ?? [],
    ...
    <div class="space-y-5 p-4">
      ...
      <div class="text-center">
        <DsText size="sm" weight="light" class="text-center">
          Provide details for the NPIs to add to this workspace and the directory.
        </DsText>
      </div>
      ...
      <div class="space-y-5 mx-5">
        ...
        <DsForm ref="formRef">
          <DsTextInput
            v-model="state.form.npis"
            required
            :rules="[
              [v => !!v, 'NPI is required'],
              [v => /\d{10}/.test(v!), 'at least one 10 digit NPI is required']
            ]"
            label="NPIs (Single, or Comma-seperated)"
          />
        ...
        <div class="flex justify-center w-full gap-5">
          ...
          <DsButton id="add-providers(s)-modal-button" title="Add Provider(s)" :color="Colors.secondary" @click="onAddProviderClick">
            Add Provider(s)
          </DsButton>
        </div>
      </div>
